...
function establecerFlujoCiclo6x2(activo){
  const input = document.getElementById("autoCiclo6x2Interno");
  const boton = document.getElementById("toggleCiclo6x2Interno");
  const estado = document.getElementById("estadoCiclo6x2Interno");
  const valor = activo ? "si" : "no";

  if (input) input.value = valor;
  if (boton) {
    boton.textContent = activo ? "Desactivar flujo" : "Activar flujo";
    boton.classList.toggle("ok", activo);
    boton.classList.toggle("light", !activo);
  }
  if (estado) {
    estado.textContent = activo
      ? "Activo para el primer turno manual."
      : "Desactivado.";
  }
}

function alternarFlujoCiclo6x2(){
  const actual = (document.getElementById("autoCiclo6x2Interno")?.value || "no") === "si";
  const siguiente = !actual;
  establecerFlujoCiclo6x2(siguiente);

  if (siguiente) {
    reiniciarCiclo6x2Inicial();
    mostrarOk("Flujo 6x2 activado. Seleccione el primer turno manual para completar el ciclo.", false);
  } else {
    mostrarOk("Flujo 6x2 desactivado.", false);
  }
}

document.addEventListener("DOMContentLoaded", () => {
  cargarConfiguracionGuardada();
  establecerFlujoCiclo6x2(false);
